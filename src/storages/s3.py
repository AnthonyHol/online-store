...
class S3Storage(BaseStorage):
    ...
    async def upload_file(
        self, key: str | None, data: bytes, content_type: str | None
    ) -> str | None:
        if key is None:
            return None
        ...
        key = get_updated_path_depending_on_os(
            os.path.join(settings().STORAGE_FILE_PATH, key)
        )

        try:
            async with self._boto3_session.client(
                "s3", endpoint_url=self._s3_endpoint_url
            ) as s3_client:
